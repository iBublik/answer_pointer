    ...
    context 'with valid params' do
      context 'user with specified email exists' do
        ...
        it 'saves verification to database' do
          expect { post :create, verification: verification_attrs }
      ...
      context 'user with specified email does not exist' do
        it 'does not save verification to datatbase' do
          expect { post :create, verification: attributes_for(:invalid_verification) }
            .to_not change(Verification, :count)
        end
  ...
  describe "GET#confirm" do
    ...
    context 'with valid token' do
      it 'deletes record from the database' do
        expect { get :confirm, id: verification, token: verification.token }
          .to change(Verification, :count).by(-1)
      end
    end
    ...
    context 'with invalid token' do
      it 'does not delete record from the database' do
        expect { get :confirm, id: verification, token: wrong_token }
          .to_not change(Verification, :count)
      end
    end
  end
